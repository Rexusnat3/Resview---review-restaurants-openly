...
class RestaurantReviewer:
    ...
    def get_average_rating(self, restaurant_name):
        """Returns the average rating by adding all reviews from the reviews list"""
    ...
    def load_reviews_from_file(self, file_name):
        """Loads all the reviews in the JSON file"""
        try:
...
class CuisinesRestaurantReviewer(RestaurantReviewer):
    """This is a subclass to restaurant reviewer that adds the ability to categorize the restaurants by cuisine'"""
    def __init__(self):
        super().__init__()
    ...
    def get_reviews_by_cuisine(self, cuisine_type):
        """Retrieves the reviews for a specific type of cuisine, returns empty list if there are none"""
        return self.categorized_reviews.get(cuisine_type, [])


# Example usage
if __name__ == "__main__":
    categorized_reviewer = CuisinesRestaurantReviewer()
